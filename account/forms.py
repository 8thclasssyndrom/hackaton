...
class SignUpForm(forms.ModelForm):
    ...
    def clean_email(self):
        ...
        if User.objects.filter(email=email).exists():
            raise forms.ValidationError('This email is already busy')
        return email
    ...
    def save(self):
        ...
        user.send_activation_mail('sign-up')
        return user
...
class ForgotPasswordForm(forms.Form):
    ...
    def clean_email(self):
        ...
        if not User.objects.filter(email=email).exists():
            raise forms.ValidationError("User isn't defined")
        return email
...
class ForgotPasswordCompleteForm(forms.Form):
    ...
    def clean_code(self):
        ...
        if not User.objects.filter(activation_code=code).exists():
            raise forms.ValidationError('Wrong activation code')
        return code

    def clean(self):
        password = self.cleaned_data.get('password')
        password2 = self.cleaned_data.get('old_password')
